lmsModule.controller("bookLoanController", function($scope, $http, $window, $route, lmsRestService, lmsConstants) {
...
//		$window.confirm(authorID);
	...
	//deleteAuthor
	$scope.deleteAuthor = function(index, authorName){
		var deleteAuthor = $window.confirm('Are you sure you want to delete Author: ' + authorName + ' ?');
		if(deleteAuthor){
			$http.post(lmsConstants.AUTHOR_DELETE, {authorId: $scope.authors[index].authorId}).success(function(data) {
				lmsRestService.get(lmsConstants.AUTHOR_VIEW, 
						function(data) {
							$scope.authors = data;
						})
